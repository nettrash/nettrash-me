...
- **QR** — QR code generator (SVG output)
- **Network** — URL parser, HTTP headers inspector
- **JWT** — JWT token decoder (header, payload, signature inspection with timestamp annotations)
- **JSON/YAML** — JSON formatter/minifier, JSON ↔ YAML converter
...
frontend/               # Yew WASM frontend
  ...
  src/
    ...
    components/
      ...
      qr.rs             #   QR code generator
      network.rs        #   Network tools (url parser, http headers)
      jwt.rs            #   JWT decoder
      json_yaml.rs      #   JSON/YAML tools (formatter, converter)
